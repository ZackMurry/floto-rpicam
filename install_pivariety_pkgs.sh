...
configCheck() {
    ...
    if [ $(grep -c "dtparam=i2c_vc=on" "$BOOTCONFIG") -eq 0 ]; then
         sed -i '$adtparam=i2c_vc=on' "$BOOTCONFIG"
         sed -i '$adtparam=i2c_arm=on' "$BOOTCONFIG"
    ...
    || $(grep -c "^dtoverlay=arducam" "$BOOTCONFIG") -ne 0 ]]; then
        ...
        'y'|'Y')
            echo "Changed"
             sed 's/^\s*dtoverlay=imx519/#dtoverlay=imx519/g' -i $BOOTCONFIG
             sed 's/^\s*dtoverlay=arducam/#dtoverlay=arducam/g' -i $BOOTCONFIG
             sed 's/^\s*dtoverlay=arducam_64mp/#dtoverlay=arducam_64mp/g' -i $BOOTCONFIG
...
openCamera() {
    if [ -n "$OpenCameraName" ]; then
         dtoverlay $OpenCameraName>/dev/null 2>&1
    fi
}
...
if [[ $VERSION_CODENAME == "bookworm" ]]; then
        ...
        #     echo "ov64a40"
            ...
            if [ $? == 0 ]; then
                ...
                if [ $package == "imx519" ]; then
                     sh -c 'echo dtoverlay=imx519 >> /boot/config.txt'
                elif [ $package == "64mp" ]; then
                     sh -c 'echo dtoverlay=arducam-64mp >> /boot/config.txt'
                elif [ $package == "kernel" ]; then
                     sh -c 'echo dtoverlay=arducam-pivariety >> /boot/config.txt'
                fi
                exit -1
            else
                echo -e "${RED}"
    ...
    if [[ $package == *"kernel_driver"* ]]; then
        ...
        if [ $package == 'libcamera_bookworm' ]; then
             apt purge libpisp0.0.1 -y
             apt install libpisp-dev -y
        fi
        ...
        if [ $package == 'libcamera_bookworm' ]; then
            ...
            wget -O $pkg_name_ipa $download_link_ipa
             apt --reinstall install -y ./$pkg_name_dev
             apt --reinstall install -y ./$pkg_name_ipa
